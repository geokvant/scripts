#!/bin/bash
wget https://raw.githubusercontent.com/geokvant/scripts/main/ssh-add-keys.sh
wget https://raw.githubusercontent.com/geokvant/scripts/main/no-password.sh
wget https://raw.githubusercontent.com/geokvant/scripts/main/docker-install.sh
wget https://raw.githubusercontent.com/geokvant/scripts/main/automatic/zerotier-join.sh
bash ssh-add-keys.sh
bash no-password.sh
